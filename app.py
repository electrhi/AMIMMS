...
import base64, os, qrcode, socket, gspread, pytz
...
def login():
    ...
    users = {row["ID"]: {"pw": row["PASSWORD"], "auth": row.get("AUTHORITY", 0)} for _, row in df.iterrows()}
    ...
    if request.method == "POST":
        ...
        if user_id in users and users[user_id]["pw"] == pw:
            ...
            session["authority"] = int(users[user_id]["auth"])
...
def menu():
    ...
    return render_template("menu.html", user_id=session["user_id"], authority=session.get("authority", 0))
...
def upload_to_gcs(file_path, file_name, bucket_name):
    try:
        creds = storage.Client.from_service_account_info(json.loads(os.environ["GOOGLE_CREDENTIALS_JSON"]))
...
def generate_receipt(materials, giver, receiver, giver_sign, receiver_sign):
    ...
    if os.path.exists(logo_path):
        logo = Image.open(logo_path).resize((220, 90))
    ...
    draw.text((100, 200), f"작성일자: {now.strftime('%Y-%m-%d %H:%M')}", font=bold_font, fill="black")
    ...
    if giver_img:
        ...
        img.paste(giver_resized, (240, footer_y - 190), giver_resized)
    if receiver_img:
        receiver_resized = receiver_img.resize((260, 120))
        img.paste(receiver_resized, (840, footer_y - 190), receiver_resized)

    tmp_filename = f"/tmp/receipt_{receiver}_{now.strftime('%Y%m%d_%H%M%S')}.jpg"
    img.save(tmp_filename, "JPEG", quality=95)

    BUCKET_NAME = os.getenv("GCS_BUCKET_NAME", "amimms-receipts")
    return upload_to_gcs(tmp_filename, os.path.basename(tmp_filename), BUCKET_NAME)
...
def admin_summary():
    ...
    if df.empty:
        return render_template("admin_summary.html", table_html=None, message="등록된 자재 기록이 없습니다.")

    df = df[df["주는사람"] == user_id]
    if df.empty:
        return render_template("admin_summary.html", table_html=None, message="해당 사용자의 기록이 없습니다.")
    ...
    table_html = pivot.to_html(classes="border-collapse border border-gray-300 w-full text-center", index=False)
